...
(define (get/indices X Y)
  (combine (build-list X (λ (x) x)) (build-list Y (λ (x) x))))

(define (solved-grid? grid size)
  (let ([board-size (* (car size) (cdr size))])
    (andmap (λ (indices) (= (length (hash-ref grid indices)) 1))
            (get/indices board-size board-size))))

(define (solved-grid->string grid size)
  (let ([board-size (* (car size) (cdr size))])
    (foldr (λ (row str)
             (string-append
              (foldr (λ (col str)
...
(define (assign grid size indices val)
...
(define (eliminate grid size indices val)
  (define (apply/eliminate-rule-1 grid val)
    (foldl (λ (peer grid)
             (if (false? grid)
                 #f
  ...
  (define (apply/eliminate-rule-2 grid)
    (foldl (λ (unit grid)
             (if (false? grid)
                 #f
...
(define (solve-cp board size)
  (call/cc
   (λ (k)
     (let ([board-size (* (car size) (cdr size))])
       (define (solve-cp-grid grid)
