...
(: flip-coin
   (-> Exact-Positive-Integer Pseudo-Random-Generator (Pairof Boolean Pseudo-Random-Generator)))
(define flip-coin
...
(define count/total-options
  (λ (grid size)
    (let ([board-size (* (car size) (cdr size))])
      (foldl (λ (indices [sum : Natural]) (+ sum (length (hash-ref grid indices))))
             0
  ...
  (λ (size)
    (let ([board-size (* (car size) (cdr size))])
      (let ([options (build-list board-size (λ ([x : Natural]) (+ x 1)))])
        (foldl (λ ([indices : Indices] [grid : Grid]) (hash-set grid indices options))
               (hash (cons 0 0) '())
               (get/indices board-size board-size))))))
...
(define grid->board
  (λ (grid size)
    (let ([board-size (* (car size) (cdr size))])
      (foldr (λ ([row : Natural] [board : Board])
               (cons (foldr (λ ([col : Natural] [lst : (Listof (U Hole Natural))])
  ...
  (λ (size)
    (let ([board-size (* (car size) (cdr size))])
      (letrec ([generate-by-indices
                :
                (-> Natural Natural Grid MaybeGrid)
                    ...
                    [else
                     (if (= 0 (random board-size))
                         (let* ([options (hash-ref grid (cons row-index col-index))]
                                [ts (foldl
                                     (λ ([val : Natural] [ts : (Listof (Pairof Natural Grid))])
                                       (let ([new-grid
                                     ...
                                     '()
                                     options)])
                           (foldr (λ ([ts : (Pairof Natural Grid)] [grid : MaybeGrid])
                                    (if (false? grid)
                                        (generate-by-indices row-index (+ 1 col-index) (cdr ts))
                                        grid))
                                  #f
                                  (sort ts
                                        (λ ([t1 : (Pairof Natural Grid)] [t2 : (Pairof Natural Grid)])
